...
hdfs_master_id=$(sudo docker run -d -v ${volume}/hdfs-namenode:/data --name hdfs-namenode -h hdfs-namenode --network=${net}  hdfs namenode start hdfs-namenode)

sleep 2s

ip=$(sudo docker inspect --format '{{ .NetworkSettings.Networks.'${net}'.IPAddress }}' ${hdfs_master_id})
...
for n in $(seq 1 1 ${nodes}); do
	...
	datanode_id=$(sudo docker run -d -v ${volume}/hdfs-datanode${n}:/data --name hdfs-datanode${n} -h hdfs-datanode${n} --network=${net} hdfs datanode start hdfs-namenode)
done
