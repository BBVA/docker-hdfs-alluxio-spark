...
alluxio_master_id=$(sudo docker run -d -v ${volume}/alluxio-master:/data --name alluxio-master -h alluxio-master --network=${net}  alluxio master start hdfs-namenode)

sleep 2s

ip=$(sudo docker inspect --format '{{ .NetworkSettings.Networks.'${net}'.IPAddress }}' ${alluxio_master_id})
...
for n in $(seq 1 1 ${nodes}); do
	...
	datanode_id=$(sudo docker run -d -v ${volume}/alluxio-worker${n}:/data --name alluxio-worker${n} -h alluxio-worker${n} --network=${net} alluxio slave start alluxio-master)
done
